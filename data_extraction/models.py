"""
Data models for GitHub events.
Using dataclasses for immutable, type-safe data structures.
"""
...
class EventType(Enum):
    """Enumeration of GitHub event types we care about."""
...
class Actor:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Actor':
        """Factory method to create Actor from API response."""
        return cls(
            id=data.get('id'),
...
class PullRequest:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'PullRequest':
        """Factory method to create PullRequest from API response."""
        return cls(
            id=data.get('id'),
...
class Comment:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Comment':
        """Factory method to create Comment from API response."""
        return cls(
            id=data.get('id'),
            body=data.get('body', ''),
            created_at=datetime.fromisoformat(data['created_at'].replace('Z', '+00:00')),
...
@dataclass(frozen=True)
class GitHubEvent:
    """Represents a GitHub event from GHArchive."""
    event_id: str
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'GitHubEvent':
        """Factory method to create GitHubEvent from GHArchive JSON."""
        return cls(
    ...
    def extract_text_content(self) -> Optional[str]:
        """
        Extract human-readable text content for sentiment analysis.
        Different event types have different text locations.
        """
        ...
        elif self.event_type in [EventType.PR_REVIEW_COMMENT, EventType.ISSUE_COMMENT]:
